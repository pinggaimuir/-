...
import cn.futures.data.importor.crawler.price21food.assist.Query21foodPrice;
import cn.futures.data.importor.crawler.price21food.assist.formatData;
import cn.futures.data.importor.crawler.price21food.assist.wholePriceAnalyse;
...
public class Price21foodDataFetch {
	 ...
	 * 定时任务
	 */
	@Scheduled(cron= CrawlScheduler.CRON_21FOOD)
	public void start(){
		logger.info("开始抓取Price21food食品商务网价格行情数据");
		fetchAndSave21foodData();
		logger.info("Price21food食品商务网价格行情数据爬虫执行完毕");
	}
...
//                                  map.put("产地规格",VarId);
	...
	public static void main(String[] a){		
        ...
        for(String dateStr:dates){
			...
			fetch.save21foodPriceData(new File(Constants.MARKETPRICEDATA_21FOOD_ROOT), dateStr);
