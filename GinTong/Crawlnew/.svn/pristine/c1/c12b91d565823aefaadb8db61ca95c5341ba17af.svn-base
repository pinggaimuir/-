...
import cn.futures.bricAnalyse.BricAnalyseData;
...
public class dayAnalyse {
	...
	public void saveData(BricAnalyseData p){
		...
		try {
			...
			jdbc.endTransaction();
			String sql = "update CFG_TABLE_META_NEW  set dataUpdateTime=getDate() where dbName like 'CX_MarketData_DayAnalyse' and varid="+p.VarId;
			jdbc.update(sql);
		}catch(Exception e){
			e.printStackTrace();
		}finally {
	...
	public  BricAnalyseData calculator(int varid){				
		...
		String sqlLast = "select top 1 * from CX_MarketData_DayAnalyse where varId = "
		+varid+" and timeint < "+timeintStart+" order by timeint desc,edittime desc";
		//获取当天数据，也有可能当天有很多条数据，以edittime取最新数据.持仓量和成交量成交额取指数连续，其他取主力连续
		int timeintHour = CrawlerUtil.nowTimeintHour();
		String sqlCurrent = "select top 1 a.timeint,a.varid,a.code,a.edittime,a.开盘价,a.最高价,a.最低价,a.收盘价,a.结算价,b.持仓量,b.成交量,b.成交额 "
				+" from CX_MarketData_2FH a,CX_MarketData_1FH b where a.timeint=b.timeint and a.varId=b.varId "
				+" and a.varid = "+varid+"  and a.timeint <= "+timeintHour+" order by a.timeint desc,a.edittime desc";		
		//获取当天现货均价，当天可能没有数据，就提取最新的报价数据
		String sqlXianhuo = "select top 1 * from "+saveTable.get(varid)+" where varId = "
		+qixianduizhao.get(varid)+" and timeint<="+(timeintHour/100)+" order by timeint desc,edittime desc";
