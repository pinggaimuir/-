...
public class ExcelImportDAO {
	...
	public void insert(String table, int timeint, int varid,
		...
		sqlBuilder.append(") VALUES ( getDate(), ").append(varid);
		for (String field : fields) {
			sqlBuilder.append(", ?");
		}
		sqlBuilder.append(")");
		...
		try {
			...
			sql = "update CFG_TABLE_META_NEW  set dataUpdateTime=getDate() where dbName like '"+table+"' and varid="+varid;
			db.update(sql);
		} catch (Exception e) {
			e.printStackTrace();
			LOG.error(e);
	...
	public ExcelDataset findRecentData(String table, int varid) {
		...
		int topnum = 10;
		String sql = "select top " + topnum + " * from " + table + " where varid=" + varid
				+ " order by EditTime desc, id desc";
		JdbcRunner db = null;
		try {
			db = new JdbcRunner();
			ResultSet rs = db.query(sql);
