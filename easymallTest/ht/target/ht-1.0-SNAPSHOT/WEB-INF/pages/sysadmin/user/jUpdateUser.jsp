...
<div id="innerMenubar">
  <div id="navMenubar">
<ul>
	<li id="new"><a href="#" onclick="formSubmit('updateUser','_self');this.blur();">保存</a></li>
...
<table id="ec_table" class="tableRegion" width="98%" >
	<tbody class="tableBody" >
		...
		<tr class="odd">
			...
			<td>入职时间</td>
			<td><input type="text" value="<fmt:formatDate value='${user.userInfo.joinDate}'/>" name="userInfo.joinDate" style="width:127px;"
		...
		<tr class="odd">
			...
			<td><input type="radio" name="userInfo.gender" value="1"
                ...
                <c:if test="${user.userInfo.gender==0}">checked</c:if>/>女
            </td>
			<td>岗位</td>
                ...
                <select name="userInfo.manager.userInfoId">
                    <c:forEach items="${userInfoList}" var="m">
                        <option value="${m.userInfoId}"
                            <c:if test="${m.userInfoId==user.userInfo.userInfoId}">
                                selected="selected"
                            </c:if>
                        >${m.name}</option>
                    </c:forEach>
