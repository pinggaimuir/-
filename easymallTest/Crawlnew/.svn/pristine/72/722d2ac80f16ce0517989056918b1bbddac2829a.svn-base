...
public class MarketPrice implements Cloneable {
	...
	private String sqlTemplate = "insert into %table% (Edittime, varId,timeint,开盘价, 最高价, 最低价, 收盘价, 结算价, 持仓量, 成交额, 成交量, code) " +
			"values(sysdate(),%variety%,%timeint%,%open%,%high%,%low%,%close%,%price%,%hold%,%totalmoney%,%totalamount%,%code%)";
	...
	public MarketPrice(String table, int varid) {
		super();
		this.table = table;
		this.varid = varid;
	}
	...
	public String toString() {
		return "MarketPrice [table=" + table + ", varid=" + varid +  ", timeint="
