...
public class CZCEFuturesMarket {
					...
					if(line.length() < 2||line.contains("郑州商品交易所期货每日行情表")||line.contains("小计")||line.contains("总计")||line.contains("品种月份")){
						...
						if(czce_varName_Map.keySet().contains(code)){
							...
							if(!monthList.contains(month)){
								...
								price.setCnName(month);
								price.setVarName(varName);
					...
					if(varId != -1){
						if(varName.equals("郑州菜籽")){
							...
							for(MarketPrice price:prices){
								price.setCnName("N"+monthMap.get(price.getCnName()));
							}
						}else{
							...
							for(MarketPrice price:prices){
								String cnNameTmp = monthMap.get(price.getCnName());
								if(cnNameTmp.startsWith("(N+1)") || cnNameTmp.startsWith("N")){
									price.setCnName(cnNameTmp);
								}else{
									price.setCnName("N"+cnNameTmp);
									MarketPrice dupPrice = new MarketPrice();
									dupPrice = price.clone();
									dupPrice.setCnName("(N+1)"+cnNameTmp);
						...
						//存储数据
						priceDao.dbm3Save(removePrices,"期货行情数据-郑州商品交易所");
					}
				}
			}
		}
	...
	public static void main(String[] args){
		...
		Date date = new Date();
		//futures.start();
		date = DateTimeUtil.parseDateTime("20170321", "yyyyMMdd");
		while(DateTimeUtil.parseDateTime("20170320", "yyyyMMdd").before(date)){
