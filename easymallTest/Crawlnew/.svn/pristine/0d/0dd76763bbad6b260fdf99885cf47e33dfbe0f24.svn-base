...
public class FoodSecurity {
	 ...
	 * @DomainCode = 'FS'
	 */
	private void fetchFoodSecurity(){
		logger.info("======开始抓取粮食安全指标数据=====");
		String comp = "'\\(([0-9|,|'|-]*)\\)',";
		Map<String, String> dataMap = new HashMap<String, String>();
		postParams.put("json", postParams.get("json").replaceAll("@DomainCode = '([^']+)',", "@DomainCode = 'FS',"));
		for(String varNameTmp:ParamsMap.faostat_food_security_index_varname2headers_map.keySet()){
			String[] varNames = varNameTmp.split("	");
			String varName = varNames[0];
			int varId = Variety.getVaridByName(varName);
			//elements
			postParams.put("json", postParams.get("json").replaceAll("@List2Codes = "+comp, "@List2Codes = '(''"+ParamsMap.elements_map.get(varNames[1])+"'')',"));
			Map<String, String> headerMap = ParamsMap.faostat_food_security_index_varname2headers_map.get(varNameTmp);
			//countries
			...
			//items
			Map<String, String> cnNameMap = ParamsMap.faostat_food_security_index_varname2cnnames_map.get(varNameTmp);
			for(String cnNameTmp:cnNameMap.keySet()){
				postParams.put("json", postParams.get("json").replaceAll("@List3Codes = "+comp, "@List3Codes = '(''"+ParamsMap.items_map.get(cnNameTmp)+"'')',"));
				...
				if(contents != null && !contents.equals("")){
					...
					for(String line:lines){
						...
						logger.info("保存"+varName+"  "+fields[1]+"  "+fields[8]);
