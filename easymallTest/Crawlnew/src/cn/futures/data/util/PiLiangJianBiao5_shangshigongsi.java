...
public class PiLiangJianBiao5_shangshigongsi {
	...
	static String sql_def="insert into CFG_TABLE_META(industryId,industryDetailId,varId,cnName,dbName,"
			...
			+ "'统计局年鉴',0,sysdate(),sysdate(),28,#sort,#timeType,#updateFreq)";
...
*/
		...
		for(int i=start;i<=end;i++){
			...
			if(region==null || region.length()<1)
				continue;
			sql = "insert into CX_Variety values (sysdate(),'"+region+"',"+hangye+")";
			...
			for(int j=start;j<=end;j++){
				...
				//查找有没有同样的表头,表复用
				String dbName;
				sql = "select dbname from cfg_table_meta as t1 where headerID="+header+" and "+varid+" not in (select distinct varid from cfg_table_meta as t2 where t2.dbname=t1.dbname) limit 1";
				rs = jdbc.query(sql);
				if(rs.next()){
