...
public class CZCEDataFetch {
	 ...
	 * 定时任务
	 */
	@Scheduled(cron= CrawlScheduler.CRON_CZCE)
	public void start(){
		logger.info("开始抓取CZCEDataFetch大连持仓量排名数据");
		fetchAndSaveCZCEData();
		logger.info("CZCEDataFetch大连持仓量排名数据爬虫执行完毕");
	}
	...
	public void fetchAndSaveCZCEData(){
		...
		fetchData(todayStr);
		saveCZCEData(new File(Constants.ZZCCDATA_ROOT), todayStr);
	}
