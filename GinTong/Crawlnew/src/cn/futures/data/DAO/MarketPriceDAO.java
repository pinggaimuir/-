...
public class MarketPriceDAO {
	 ...
	 * @param industry
     */
	public void dbm3Save(List<MarketPrice> priceList,String industry){
		for(MarketPrice mp:priceList) {
			...
			list.add(map);
			data.put("tableDir", industry + "-" + mp.getVarName() + "-" + mp.getCnName());
			data.put("data", list);
			dbm3Dao.saveByTableDir(data);
		}
	}
	 ...
	 * @param industry
	 */
	public void dbm3SaveNoCode(List<MarketPrice> priceList,String industry){
		for(MarketPrice mp:priceList) {
			List list = new ArrayList<>();
			Map<String, String> map = new HashMap<>();
			Map data = new HashMap<>();
	...
	public List<MarketPrice> list(String table){
		...
		String sql = "select top 50 * from " + table 
				+ " order by edittime desc, timeint desc, varid asc";
...
//			vals[i][1] = p.getTimeint();
	...
	public void saveNoCode(List<MarketPrice> list){
		...
		Map<String, List<MarketPrice>> tableMap = new HashMap<String, List<MarketPrice>>();
		for (MarketPrice p : list){
			if (p == null || p.getCnName() == null) continue;
			
			List<MarketPrice> slice = tableMap.get(p.getCnName());
			if (slice == null){
				slice = new ArrayList<MarketPrice>();
			} 
			slice.add(p);
			tableMap.put(p.getCnName(), slice);
		}
		// do batch
		for (String cnName : tableMap.keySet()){
			saveNoCode(tableMap.get(cnName), cnName);
			LOG.info("完成向cnName："+cnName+" 内存入数据");
		}
	}
