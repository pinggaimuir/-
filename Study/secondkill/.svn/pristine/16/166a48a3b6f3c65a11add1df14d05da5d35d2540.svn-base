	...
	public void save(Price p,int varid){
		...
		try {
			...
			if(Integer.valueOf(p.timeint)>maxTotal){	
				...
				String sql = "update CFG_TABLE_META_NEW set dataUpdateTime=sysdate() where dbName like 'CX_agri_prod_wholesale_prices_totalindex' and varid="+varid;
				jdbc.update(sql);
			}
			if(Integer.valueOf(p.timeint)>maxClz){	
				...
				String sql = "update CFG_TABLE_META_NEW set dataUpdateTime=sysdate() where dbName like 'CX_clz_prod_wholesale_price_index' and varid="+varid;
				jdbc.update(sql);
			}
			jdbc.endTransaction();
		} catch (Exception e){
