...
public class DAOUtils {
	...
	Dbm3InterfaceDao dbm3Dao=new Dbm3InterfaceDao();
	//取最近的主力连续code
	public String getNewestMajorCode(int varId){
		String query = "select code from CX_MarketData_2  where varId = "+varId+" order by TimeInt desc limit 1";
	...
	public static int getNewestTimeInt(String tableName){
		String query = "select timeint from TABLE order by timeint desc limit 1";
		JdbcRunner jdbc = null;
		int timeInt = 0;
		try{
			ResultSet rs;
	...
	public String getNewestDataByProv(int varId, String cnName, String prov ){
		...
		String query = "select "+prov+" from " + tableName + " where varid=" + varId +
			" order by timeInt desc limit 1";
	...
	private void updateOrSave(String varName, int varId, String cnName, String tableName, int timeInt, Map<String, String> dataMap, Integer crawlerId){
		...
		dataMap.put("TimeInt",timeInt+"");
		dataMap.put("VarId",varId+"");
		list.add(dataMap);
		data.put("data",list);
	...
	public void batchSaveByDataMap(int varId, String cnName, Map<Integer, Map<String, String>> time2dataMap, Integer crawlerId){
		...
		if(tableName.equals("")){
			...
			Map data=new HashMap<>();
			List list=new ArrayList<>();
			for(Map.Entry<Integer,Map<String,String>> entry:time2dataMap.entrySet()) {
				Map<String,String> dataMap=entry.getValue();
				dataMap.put("TimeInt", entry.getKey() + "");
				dataMap.put("VarId", varId + "");
				logPrint(null, varId, cnName, tableName, "a", entry.getKey(), dataMap);//记录数据更新日志
				list.add(dataMap);
			}
			data.put("data",list);
			data.put("dbName",tableName);
	...
	*/
	...
	private void execInsert(String sql, Object[][] vals,String table, int varId, Integer crawlerId){
		...
		try {
			...
			jdbc.endTransaction();
			if(crawlerId == null){
				sql = "update CFG_TABLE_META_NEW set dataUpdateTime=sysdate() where dbName = '"+table+"' and varid="+varId;
				jdbc.update(sql);
			} else {
				sql = "update CFG_TABLE_META_NEW set dataUpdateTime=sysdate(), crawlerId = ? where dbName = ? and varid=?";
	...
	public void saveOrUpdateForMarketPrice(String productName, String marketName, Integer timeInt, Map<String, String> priceInfo){
		...
		if(count > 0){//已有，更新
			String updateSql = "UPDATE market_price SET EditTime = sysdate(), min_price = ?, max_price = ?, ave_price = ?, produce_area = ?, specification = ? WHERE product_id = ? AND market_id = ? AND TimeInt = ? AND specification = ?";
