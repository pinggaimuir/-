...
public class LondonSugarFetch {
	...
	private final String INDUSTRY="期货行情数据-伦敦LIFFE";
	...
	private void fetchData(Date date){
		...
		for(String varName:LONDON_SUGAR_MAP.keySet()){
			...
			if(contents == null){
				...
				priceList = getListByContents(contents, varName);
			}
			//不包括code字段的价格数据保存
			dao.dbm3SaveNoCode(priceList,INDUSTRY);
			//备份至txt
			DAOUtils.bak2txt(priceList);
		}
	}
	
	private List<MarketPrice> getListByContents(String contents, String varName) {
		...
		for(int i=0; i<SERIES_CONSTANT; i++){
			...
			String cnName = (i+1) + "号连续";
			logger.info("read[" + cnName + "]["+varName+"]:"+line);
			dataFetchUtil.addList(prices, parse(line, cnName, varName));
		}
		return prices;
	}
	 ...
	 * @return
	 */
	private MarketPrice parse(String line, String cnName, String varName){
		MarketPrice p = new MarketPrice(cnName, varName);
		try {
