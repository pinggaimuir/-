...
public class PiLiangJianBiao {
	...
	static String sql_def="insert into CFG_TABLE_META_NEW(industryId,industryDetailId,varId,cnName,dbName,"
			...
			+ "'统计局年鉴',0,sysdate(),sysdate(),28,#sort,0)";
	...
	static String sql_def2="insert into CFG_TABLE_META_NEW(industryId,industryDetailId,varId,cnName,dbName,"
			...
			+ "'#datasource',0,sysdate(),sysdate(),#creatorid,#sort,0)";
...
//				System.out.println(sql);
		...
		for(int i=start;i<=end;i++){
			...
			if(rs.next()){
				...
				String sql2 = "insert into CX_Variety_new (EditTime, VName, industryDetailId) values (sysdate(),'"+region+"',"+zihangye+")";
				log.info("添加品种：" + sql2);
				jdbc.update(sql2);	
				sql = "select id from CX_Variety_new where vname like '"+region+"'";
				log.info("品种查询：" + sql);
			...
			for(int j=start;j<=end;j++){
				...
				sql = "select id from cfg_table_meta_new where varId = " + varid + " and cnName = '" + cnName +"' limit 1";
				rs = jdbc.query(sql);
				...
				//查找有没有同样的表头,表复用
				String dbName;
				sql = "select dbname from cfg_table_meta_new as t1 where headerID="+header+" and "+varid+" not in (select distinct varid from cfg_table_meta_new as t2 where t2.dbname=t1.dbname) and industryid>8 limit 1";
				rs = jdbc.query(sql);
				if(rs.next()){
